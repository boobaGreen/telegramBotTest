...
bot.start(function (ctx) {
  return ctx.reply(
    "This message show when you use the /start command on the bot"
  );
});
bot.help(function (ctx) {
  return ctx.reply(
    "This message show when you use the /help command on the bot"
  );
});
bot.on("message", function (ctx) {
  ...
  ctx.reply(
    "La dimensione del messaggio \u00E8 di "
      .concat(messageSizeKB, " KB. Totale messaggi: ")
      .concat(totalMessages, ". Peso totale: ")
      .concat(
        totalSizeKB.toFixed(3),
        " e le emissioni di CO2 associate sono di "
      )
      .concat(emissions, " kg.")
  );
  ctx.reply(
    "Contenuto totale di ctx.message:".concat(JSON.stringify(ctx.message))
  );
});
bot.launch();
